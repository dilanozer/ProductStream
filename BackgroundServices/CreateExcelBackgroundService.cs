...
using ProductStream.Web.Hubs;
using ProductStream.Web.Models;

namespace ProductStream.Web.BackgroundServices
{
    public class CreateExcelBackgroundService(
        Channel<(string userId, List<Product> products)> channel, IFileProvider fileProvider,
        ...
        : BackgroundService
    {
        ...
        private DataTable GetTable(string tableName, List<Product> products)
        {
            ...
            products.ForEach(x =>
            {
                table.Rows.Add(x.Id, x.Name, x.Price, x.Description, x.UserId);
            });
